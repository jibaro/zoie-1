...
public class HourglassDirectoryManagerFactory<V extends ZoieVersion> {
	public static final Logger log = Logger
			.getLogger(HourglassDirectoryManagerFactory.class);
	...
	public HourglassDirectoryManagerFactory(File root,
			HourGlassScheduler scheduler,
		...
		log.info("starting HourglassDirectoryManagerFactory at " + root
				+ " --- index rolling scheduler: " + _scheduler + " mode: "
				+ _mode);
		updateDirectoryManager();
	}

	public HourglassDirectoryManagerFactory(File root,
			HourGlassScheduler scheduler, DIRECTORY_MODE mode) {
		_root = root;
		_scheduler = scheduler;
		this._mode = mode;
		log.info("starting HourglassDirectoryManagerFactory at " + root
	...
	protected void setNextUpdateTime() {
		...
		log.info("setNextUpdateTime: "
				+ _scheduler.getFolderName(_nextUpdateTime));
	}

	/**
	 * @return true if the current index accepting updates is changed. This
	 *         method should be paired with clearRecentlyChanged() to clear the
	 *         flag.
	...
	public boolean updateDirectoryManager() {
		...
		try {
			log.info("rolling forward with new path: "
					+ _location.getCanonicalPath());
			System.out.println("rolling forward with new path: "
					+ _location.getCanonicalPath());
		} catch (IOException e) {
			log.error(e);
		}
		_currentDirMgr = new DefaultDirectoryManager<V>(_location,
				_zoieVersionFactory, _mode);
	...
	/**
	 * @return a list that contains all the archived index directories excluding
	 *         the one currently accepting updates.
	 */
	public List<Directory> getAllArchivedDirectories() {
		...
		for (File file : files) {
			...
			log.debug("getAllArchivedDirectories: " + name + " "
					+ (file.equals(_location) ? "*" : ""));
			Calendar time = null;
			try {
				time = getCalendarTime(name);
			} catch (ParseException e) {
				log.warn("potential index corruption. we skip folder: " + name,
						e);
				continue;
			}
			if (time.before(threshold)) {
				log.info("getAllArchivedDirectories: skipping " + name
						+ " for being too old");
				continue;
			}
			...
			try {
				...
				if (!file.equals(_location)) { // don't count the current one
					...
					if (sig != null) {
						...
						log.error("potential index corruption: indexSignature not in "
								+ _location);
					}
				}
			} catch (ParseException e) {
	...
	public IndexSignature<V> getIndexSignature(File file) {
		...
		IndexSignature<V> sig = DefaultDirectoryManager.readSignature(
				directoryFile, _zoieVersionFactory);
		return sig;
	}

	public void saveIndexSignature(File tgt, IndexSignature<V> sig)
			throws IOException {
