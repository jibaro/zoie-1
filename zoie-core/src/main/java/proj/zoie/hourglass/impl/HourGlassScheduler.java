...
public class HourGlassScheduler {
	public static final Logger log = Logger.getLogger(HourGlassScheduler.class
			.getName());
	...
	public HourGlassScheduler(FREQUENCY freq, String schedule) {
		...
		log.info("schedule: " + Arrays.toString(_params) + " frequenty: "
				+ _freq);
	}
	...
	public HourGlassScheduler(FREQUENCY freq, String schedule, int trimThreshold) {
		...
		log.info("schedule: " + Arrays.toString(_params) + " frequenty: "
				+ _freq + " trimThreshold: keep last " + _trimThreshold
				+ " rolling periods");
	}
	...
	private int parseParam(String param) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Failed to instantiate HourGlassScheduler", e);
		}
		return ret;
	}
	...
	Calendar getNextRoll() {
		...
		switch (_freq) {
		case MINUTELY: // set the second so that every minute we switch over at
						// the same second
			next.set(Calendar.SECOND, _params[0]);
			if (!next.after(now))
				next.add(Calendar.MINUTE, 1);
			break;
		case HOURLY: // set the second and minute so that we switch over at the
						// same time in the hour
			next.set(Calendar.SECOND, _params[0]);
			next.set(Calendar.MINUTE, _params[1]);
			if (!next.after(now))
				next.add(Calendar.HOUR_OF_DAY, 1);
			break;
		case DAILY: // set hour, minute, second so that we switch over at the
