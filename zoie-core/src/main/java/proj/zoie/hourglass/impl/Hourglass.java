...
public class Hourglass<R extends IndexReader, D, V extends ZoieVersion>
		implements Zoie<R, D, V> {
	...
	public volatile long SLA = 4; // getIndexReaders should return in 4ms or a
									// warning is logged

	public Hourglass(HourglassDirectoryManagerFactory<V> dirMgrFactory,
			ZoieIndexableInterpreter<D> interpreter,
			IndexReaderDecorator<R> readerDecorator, ZoieConfig<V> zoieConfig) {
		_zConfig = zoieConfig;
		...
		_readerMgr = new HourglassReaderManager<R, D, V>(this, _dirMgrFactory,
				_decorator, loadArchives());
	...
	protected List<ZoieIndexReader<R>> loadArchives() {
		...
		for (Directory dir : dirs) {
			...
			try {
				...
				ZoieMultiReader<R> zoiereader = new ZoieMultiReader<R>(reader,
						_decorator);
		...
		log.info("load " + dirs.size() + " archived indices of "
				+ getSizeBytes() + " bytes in "
				+ (System.currentTimeMillis() - t0) + "ms");
		return archives;
	}

	ZoieSystem<R, D, V> createZoie(DirectoryManager<V> dirmgr) {
		return new ZoieSystem<R, D, V>(dirmgr, _interpreter, _decorator,
				_zConfig);
	}
	...
	/**
	 * return a list of ZoieIndexReaders. These readers are reference counted
	 * and this method should be used in pair with
	 ...
	 * reference counting correctly. <br>
	 * If this indexing system is already shut down, then we return an empty
	 * list.
	 * 
	...
	public List<ZoieIndexReader<R>> getIndexReaders() throws IOException {
		...
		try {
			...
			try {
				...
				if (t0 > SLA) {
					log.warn("getIndexReaders returned in " + t0
							+ "ms more than " + SLA + "ms");
				}
				return rlist;
			} finally {
	...
	/**
	 * not thread safe. should be properly lock. Right now we have two places to
	 * use it and locked by the shutdown lock. If it gets more complicated, we
	 * should use separate lock.
	 * 
	 * @throws IOException
	 */
	...
	public void returnIndexReaders(List<ZoieIndexReader<R>> readers) {
		...
		t0 = System.currentTimeMillis() - t0;
		if (t0 > SLA) {
			log.warn("returnIndexReaders returned in " + t0 + "ms more than "
					+ SLA + "ms");
		}
	}
	...
	public void consume(Collection<DataEvent<D, V>> data) throws ZoieException {
		try {
			_consumeLock.lock(); // one at a time so we don't mess up during
									// forward rolling.
									// also we cannot do two consumptions at the
									// same time anyway.
			try {
				_shutdownLock.readLock().lock();
				...
				if (_isShutdown) {
					...
					return; // if the system is already shut down, we don't do
							// anything.
				}
	 ...
	 */
	public V getVersion() {
		// _currentVersion = Math.max(_currentVersion,
		// _currentZoie.getCurrentVersion());
		if (_currentZoie.getCurrentVersion() != null) {
			if (_currentVersion == null) {
				return _currentVersion = _currentZoie.getCurrentVersion();
			} else {
				_currentVersion = _currentZoie.getCurrentVersion().compareTo(
						_currentVersion) < 0 ? _currentVersion : _currentZoie
						.getCurrentVersion();
			}
		}
	...
	public StandardMBean getStandardMBean(String name) {
		if (name.equals(HOURGLASSADMIN)) {
			try {
				return new StandardMBean(new HourglassAdmin(this),
						HourglassAdminMBean.class);
