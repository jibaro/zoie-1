...
public class DefaultDirectoryManager<V extends ZoieVersion> implements
		DirectoryManager<V> {
	public static final Logger log = Logger
			.getLogger(DefaultDirectoryManager.class);
	...
	public DefaultDirectoryManager(File location,
			ZoieVersionFactory<V> zoieVersionFactory, DIRECTORY_MODE mode) {
		if (location == null)
			throw new IllegalArgumentException("null index directory.");
		_zoieVersionFactory = zoieVersionFactory;
		_location = location;
	...
	public static <V extends ZoieVersion> IndexSignature<V> readSignature(
			File file, ZoieVersionFactory<V> zoieVersionFactory) {
		if (file.exists()) {
			...
			try {
				...
				if (fin != null) {
					try {
						...
						log.warn("Problem closing index directory file: "
								+ e.getMessage());
					}
				}
			}
	...
	public static <V extends ZoieVersion> void saveSignature(
			IndexSignature<V> sig, File file) throws IOException {
		if (!file.exists()) {
	...
	public static <V extends ZoieVersion> IndexSignature<V> getCurrentIndexSignature(
			File idxDir, ZoieVersionFactory<V> zoieVersionFactory) {
	...
	public void setVersion(V version) throws IOException {
		...
		File directoryFile = new File(_location, INDEX_DIRECTORY);
		IndexSignature<V> sig = readSignature(directoryFile,
				_zoieVersionFactory);
	...
	public boolean transferFromChannelToFile(ReadableByteChannel channel,
		...
		FileChannel fc = null;
		try {
			log.info("transferFromChannelToFile for " + fileName + " with "
					+ dataLen + " bytes");
			...
			long position = 0;
			do {
				position += fc.transferFrom(channel, position, dataLen
						- position);
	...
	public long transferFromFileToChannel(String fileName,
		...
		try {
			...
			log.info("transferFromFileToChannel for " + fileName + " of "
					+ dataLen + " bytes");
			...
			do {
				position += fc
						.transferTo(position, dataLen - position, channel);
			} while (position < dataLen);
